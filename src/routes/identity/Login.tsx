...
const Login = () => {
    ...
    const onSubmit = async (event: MouseEvent) => {
        ...
        if (setJwtResponse){
            localStorage.setItem("authToken", jwtData.jwt);
    ...
    const changePasswordVisibility = () => {
        setInput(prevValues => ({
            ...prevValues,
            passwordVisible: !prevValues.passwordVisible,
        }));
    };

    return (
        <LoginFormView values={values} handleChange={handleChange} onSubmit={onSubmit} validationErrors={validationErrors} changePassswordVisability={changePasswordVisibility} />
    );
}
